...
| Arquivo | O quê |
|---|---|
| `config/rules.ts` | **Regras**: post(s) + palavra(s)-chave → texto da DM + link + frases públicas |
...
| `app/admin?key=…` | Painel: conta conectada, regras, posts recentes (com Media ID) e log |
...
Edite `config/rules.ts`, faça commit/push (a Vercel publica sozinha):

```ts
{
  id: "advogado-2026-10-01",
  posts: ["https://www.instagram.com/p/XXXXXXXX/"],
  keywords: ["ADVOGADO"],
  link: "https://...notion.site/...",
  dm: "Oi! Aqui está o material 👇\n\n{link}",
}
```
...
4. Em **Set up Instagram business login → Business login settings → OAuth redirect URIs**, adicione `https://SEU-DOMINIO/api/auth/instagram/callback`.
   Depois do deploy, abra `/admin?key=ADMIN_SECRET` → **Conectar Instagram** → entre com @d.ia.riamente. O token de 60 dias e o ID da conta ficam salvos no Redis, e a varredura renova o token sozinha.
...
### 5. Teste
- Abra `https://SEU-DOMINIO/admin?key=ADMIN_SECRET`: tem que aparecer a conta e os posts.
- Rode a varredura: `curl "https://SEU-DOMINIO/api/cron/sweep?key=ADMIN_SECRET"`. Com `DRY_RUN=true`, o log mostra o que **seria** enviado.
